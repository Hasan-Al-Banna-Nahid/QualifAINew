...
class ApifyService {
  ...
  constructor() {
    this.apiToken = process.env.NEXT_PUBLIC_APIFY_API_TOKEN || "";
    if (!this.apiToken) {
   ...
   */
  private performSEOAnalysis(data: ApifyWebsiteContentResult): SEOAnalysisData {
  ...
  private analyzeHeadings(
    html: string
  ): SEOAnalysisData["content"]["headings"] {
    return {
  ...
  private performWordPressAnalysis(
    data: ApifyWebsiteContentResult
  ): WordPressAnalysisData {
    const html = data.html || "";
